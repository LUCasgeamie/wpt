...
async function setupEme(t, video) {
  ...
  function handleEncrypted(event) {
    ...
    session.generateRequest(event.initDataType, event.initData).catch(e => {
...
async function runEncryptedChunksTest(t) {
  ...
  for (i = 0; i < vp9.frames.length; i++, next_timestamp += frame_duration) {
    ...
    let frame_data =
        new Uint8Array(buffer, frame_metadata.offset, frame_metadata.size);
    let encrypted_frame_data = await window.crypto.subtle.encrypt(
    ...
    await sourceBuffer.appendEncodedChunks(new EncodedVideoChunk({
      ...
      data: encrypted_frame_data,
      decryptConfig: {
        encryptionScheme: 'cenc',
        keyId: testKeyId,
        initializationVector: testIV,
        subsampleLayout: [{clearBytes: 0, cypherBytes: frame_metadata.size}],
      }
    }));
  }
